$(function(){
    ...
    $(document).on('click', '.tab-cong', function(){
        ...
        $.ajax({
            ...
            method:'post',
            data:{'acao':'ticketsCongelados','perm':'admin'}
        }).done(function(data){
    ...
    $(document).on('click', '.menu-btn', function(){
        ...
        if(width <= 1000){
            ...
            if($('.menu').css('left') == '0px'){//ta aberto e com tamanho de pc
                $('header').animate({'left':'0'}).css('width','100%')
                $('.container-contentWrapper').animate({'left':'0'}).css('width','100%')
                $('.menu').animate({'left':'-300px'})
            }else{//nao esta abertto e com tamanho de pc
                $('header').animate({'left':'300'}).css('width','calc(100% - 300px)')
                $('.container-contentWrapper').animate({'left':'300'}).css('width','calc(100% - 300px)')
                $('.menu').animate({'left':'0'})
            }
        }
        
    })
    $('#logout').click(function(){ // deslogar
        //vai deslogar :c
        document.cookie = "admin_loginToken=; expires=Thu, 01 Jan 1900 00:00:00 UTC; path=/;";
        location.href=BASE+'dashboard'
    })

    $('#edit').click(function(){ // clicar pra editar perfil
        location.href=BASE+'dashboard/perfil'
    })
    $('#img-edit').click(function(){
        
    })
    $('.infoPerfil').find('.img').hover(
        function(){
    ...
    $('.avatarArea').click(function(){
        if(openAvatar){
            $('.menuHoverAvatar').css('right','-9999px').css('top','40px').animate({'opacity':'0'})
            $('.avatarArea').removeClass('hover')
            openAvatar = false;
        }else{
            $('.menuHoverAvatar').css('right','15px').css('top','40px').animate({'opacity':'1'})
...
function alertar(texto,redirect){
    ...
    $('.message-alert-box').animate({'opacity':'1'}).removeClass('error').css('right','50px')
    setTimeout(function(){
        $('.message-alert-box').animate({'opacity':'0'}).css('right','-400px')
        $('.content-Alert').text('')
        if(redirect != '' || redirect.length > 0){location.href=redirect}
    }, 5000)
}
...
function activity(){
    $.ajax({
        url:BASE+'ajax/Admin.php',
        method:'post',
        data:{'type-action':'updateActivity'}
    })
}
