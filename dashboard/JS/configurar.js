$(function(){
    ...
    $(document).on('click', '[js="tab-adicionarNovoAdmin"]', () => {
        $('#fountainG').css('display','block')
        $.ajax({
            url:BASE+'ajax/Admin.php',
            method:'post',
            data:{'type-action':'tab-adicionarAdmin'}
        }).done(function(data){
    ...
    $(document).on('click', '[js="confirm-adicionarNovoAdmin"]', () => {
        var user = $(this).find('input[name="username"]').val()
        var email = $(this).find('[name="email"]').val()
        var chavemestra = $(this).find('[name="code"]').val()

        if(chavemestra.length <= 0 || email.length <= 0 || user.length <= 0 ){return alertar('Você precisa preencher todos os dados.','')}
        $('#fountainG').css('display','block')
        $.ajax({
            url:BASE+'ajax/Admin.php',
            method:'post',
            data:{'type-action':'adicionarAdmin','username':user,'email':email,'chavemestra':chavemestra}
        }).done(function(data){
            $('#fountainG').css('display','none')
            data = JSON.parse(data)
            alertar(data.msg,'')
    ...
    $(document).on('click', '[js="changeCodeMaster"]', function(){
        ...
        $.ajax({
            url:BASE+'ajax/Admin.php',
            method:'post',
            data:{'type-action':'tab-changeCodeMaster'}
        }).done(function(data){
            data = JSON.parse(data)
            $('#fountainG').css('display','none')
            $('.tabopened').remove()
    ...
    $(document).on('click', '[js="confirm-changeCodeMaster"]', function(){
        ...
        $.ajax({
            url:BASE+'ajax/Admin.php',
            method:'post',
            data:{'type-action':'changeCodeMaster','codeantigo':codeantigo,'codenovo':codenovo}
        }).done(function(data){
            data = JSON.parse(data)
            $('#fountainG').css('display','none')
            alertar(data.msg,'')
