<html>
    <head>
        <title><?php echo $this->info['titulo'];?></title>
        <meta name="description" content="<?php echo $this->info['desc'];?>">
        <meta name="keywords" content="<?php echo $this->info['tags'];?>">
        <meta name="author" content="matwcode">
        <meta name="googlebot" content="noindex">
        <meta name="viewport" content="width=device-width, initial-scale=1">
        <link rel="stylesheet" type="text/css" href="<?php echo BASE;?>dashboard/Views/styles/style.css">
        <link rel="shortcut icon" href="<?php echo BASE;?>favicon.ico">
        <link rel="preconnect" href="https://fonts.gstatic.com">
        <link href="https://fonts.googleapis.com/css2?family=Roboto+Condensed:ital,wght@0,300;0,400;0,700;1,300;1,400;1,700&display=swap" rel="stylesheet">
        <BASE base="<?php echo BASE?>">
	<?php
      Site::loadJs(
      'admin',
      'user',
      'all', 
      'jquery'
      );
      Site::loadJs(
            'admin',
            'user',
            'all', 
            'constants'
            );
      Site::loadJs(
      'admin',
      'admin',
      'all', 
      'functions'
      );
      Site::loadJs('admin','admin',array('chat'),'chat');
      Site::loadJs('admin','user','all','constants'); 
      Site::loadJs('admin','user','all','jquery.form'); 
      Site::loadJs('admin','admin',array('admin'),'adminpage'); 
      Site::loadJs('admin','admin','all','jquery.maskMoney');
      Site::loadJs('admin','admin',array('chat'),'tickets');
                
     
?>
    </head>
    <body>
        <header>
            <nav>
                <div class="menu-btn"><i class="fa fa-bars" aria-hidden="true"></i></div>
                <div class="avatarArea right"><img src="<?php echo Admin::getProfilePhoto(); ?>"><span><?php echo Admin::getUserInfo($_COOKIE['admin_loginToken'])['username']; ?> </span></div>
                <div class="clear"></div>
            </nav>
        </header>
        <div class="message-alert-box">
            <h2>Notificação</h2>
            <div class="content-Alert"></div>
        </div>
        <div id="fountainG">
            <div style="position:relative; left:20%; top:30%">
                <div id="fountainG_1" class="fountainG"></div>
                <div id="fountainG_2" class="fountainG"></div>
                <div id="fountainG_3" class="fountainG"></div>
                <div id="fountainG_4" class="fountainG"></div>
                <div id="fountainG_5" class="fountainG"></div>
            </div>
        </div>
        